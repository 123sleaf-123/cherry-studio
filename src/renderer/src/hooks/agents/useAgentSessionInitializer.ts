...
import type { ListAgentSessionsResponse } from '@shared/data/types/agent'
...
export const useAgentSessionInitializer = () => {
  ...
  const initializeAgentSession = useCallback(async (agentId: string) => {
    ...
    try {
      ...
      const result = (await dataApiService.get(`/agents/${agentId}/sessions`)) as ListAgentSessionsResponse
